...
object Settings {
  ...
  lazy val clientSettings =
    applicationSettings ++ sharedSettings ++ Seq(
  ...
  lazy val ReleaseCmd = Command.command("release") { state =>
    "set elideOptions in client := Seq(\"-Xelide-below\", \"WARNING\")" :: "client/clean" :: "client/test" :: "server/clean" :: "server/test" :: "server/dist" :: "set elideOptions in client := Seq()" :: state
  }
}

object dependencies {
  def sharedDependencies = Def.setting(
      Seq(
  ...
  def serverDependencies = Def.setting(
      Seq(
          // Scalaxb
  ...
  def clientDependencies = Def.setting(
      Seq(
  ...
  def jsDependencies = Def.setting(
      Seq(
